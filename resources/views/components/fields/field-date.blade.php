<div class="form-group">
    ...
    <div class="input-group">
        <input type="text" class="form-control" name="{{ $field }}" id="{{ isset($id) ? $id : $field }}" aria-describedby="{{ $field }}Help" placeholder="insert {{ $label }} here" autocomplete="off" value="@if(isset($value) && !isset($model)) {{ $value }} @else {{ old($field, isset($model) ? $model->$field : '') }} @endif" @if(isset($required)) parsley-trigger="change" required @endif @if(isset($disabled) && ($model->id)) disabled @endif>
