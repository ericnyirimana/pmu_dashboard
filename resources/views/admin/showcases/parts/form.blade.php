<div class="d-flex flex-row row mb-5">
    ...
    <div class="col-12" style="@if(!$showcase->id || $showcase->type != 'timeslots') display:none @endif" id="mealtypes">
        <field-tags id="mealtypes_field" label="Mealtype" field="mealtypes" :values="$showcase->mealtypes->pluck('translate.name')" :list="$mealtypes" required disabled="{{ !$showcase->id || $showcase->type != 'timeslots' ? 'true' : 'false' }}" />
    </div>
</div>
...
@push('scripts')
    <script>
			$(document).ready(function(){
              ...
              @if (!$showcase->id || $showcase->type != 'timeslots')
